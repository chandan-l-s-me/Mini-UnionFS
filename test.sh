...
echo "base only content" > "$LOWER_DIR/base.txt"
echo "to be deleted" > "$LOWER_DIR/delete_me.txt"
...
if grep -q "base only content" "$MOUNT_DIR/base.txt"; then
...
echo "modified content" >> "$MOUNT_DIR/base.txt" 2>/dev/null

if [ $(grep -c "modified content" "$MOUNT_DIR/base.txt" 2>/dev/null) -eq 1 ] &&
   [ $(grep -c "modified content" "$UPPER_DIR/base.txt" 2>/dev/null) -eq 1 ] &&
   [ $(grep -c "modified content" "$LOWER_DIR/base.txt" 2>/dev/null) -eq 0 ]; then
  echo -e "${GREEN}PASSED${NC}"
else
  echo -e "${RED}FAILED${NC}"
fi
